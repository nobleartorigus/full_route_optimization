...
m.save('src/views/routes_algorithm/maps/Rutas Sugeridas Urbvan Monterrey.html')
...
# kmeans = KMeans(n_clusters=len(cluster_centers), init = cluster_centers, n_init=1).fit(coords)
# cluster_labels = kmeans.labels_
# labels = unique(cluster_labels)
# clusters = Series([np.asarray(coords)[where(cluster_labels == label)[0]] for label in labels])
# cluster_centers = np.asarray([get_centermost_point(cluster) for cluster in clusters])
